...
public class Mat
{
    ...
    public Mat DotProduct(Mat mat2)
    {
        if(rows == mat2.cols)
        {
            Mat output = new Mat(rows, mat2.cols);
    ...
    public Mat GetMinor(int[,] mat, int curRow, int curCol)
    {
        Mat minor = new Mat(new int[rows - 1, cols - 1]);
        int currMinorRow = 0;
        int currMinorCols = 0;
...
public class Matrice : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        /*        matA.SwapCols(2, 1);
                ...
                matB.SwapRows(0, 1);
                Debug.Log(matB.PrintMat());*/
        /*        Mat matA = new Mat(new int[,] { {1, 2, 3, 4}, { 1, 2, 3, 4 } });
                matA.SetInverseDiagonal(11);
                Debug.Log(matA.PrintMat());*/
                //Debug.Log(matA.Determinant());

                Mat matB = new Mat(new int[,] { {-1, -2, 3, 2 },
            {0, 1, 4, -2 },
            {3, -1, 4, 0 },
            {2, 1, 0, 3 }});
                Debug.Log(matB.Determinant());

/*        Mat matA = new Mat(new int[,] { { 1, 2, 4},
                                        { 1, 4, 1},
                                        { 4, 2, 3},
                                        { 11, 3, 3},
                                        { 1, 3, 10},
